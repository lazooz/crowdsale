...
//            if (originalAmount <= satoshi) {
    ...
    static processBlock(currentBlock) {
		...
		if (inputAddresses.contains(serviceAddress))  { continue } 
                ...
                log4j.info("Block ${currentBlock} found service call: ${currentBlock} ${txid} ${inputAddresses} ${outputAddresses} ${inAmount/satoshi} ${asset.nativeAssetName} -> ${outAsset} )")
                //}
            }
        ...
        try {
            for (transaction in parsedTransactions) {
                ...
                log4j.info("insert into payments values (${payment.currentBlock}, ${payment.txid}, ${payment.sourceAddress},  ${Asset.NATIVE_TYPE}, ${payment.inAmount}, ${payment.destinationAddress}, ${payment.outAsset}, ${payment.outAssetType}, 0,  ${payment.status}, ${payment.lastModifiedBlockId})")
