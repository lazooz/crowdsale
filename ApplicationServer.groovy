...
/*
            ...
            log4j.info("Application Server get 1")
            if(form == null)
             {
               form = new Form()
                response.attributes.'org.restlet.http.headers' = form
             }
            log4j.info("Application Server get 2")
             form.add( 'Access-Control-Allow-Origin', "*")
             log4j.info("Application Server get 3")
             
  */           
             
             /*
            Form responseHeaders = new Form(); 
            log4j.info("Application Server get 4")
            responseHeaders.add("Access-Control-Allow-Origin", "*")
            log4j.info("Application Server get 5")
            response.getAttributes().put("org.restlet.http.headers", responseHeaders)
             log4j.info("Application Server get 6")
             */
...
//			router.attach("/get_balances", GetBalancesResource.class)			
    ...
    static init() {
		...
		DBCreator.createDB(db)
			...
			//expectedSecret = iniConfig.secret
