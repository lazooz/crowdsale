...
class MastercoinAPI {
	...
	private sendRPCMessage(command, in_params) {
        ...
        def result = mastercoinHttpAsync.request( POST, JSON) { req ->
 	    ...
 	    response.failure = { resp,json -> 
			log4j.info(command + " failed: ${json}") 
    ...
    public getSends(Long blockId, String address) {
		if (address == null || address.length() == 0) {
			address = "*"
		}
		return sendRPCMessage('listtransactions_MP', [address, 10000, 0, blockId,blockId])
    ...
    public sendAsset(sourceAddress, destinationAddress, asset, amount) {
		return sendRPCMessage('send_MP', [sourceAddress, destinationAddress, Long.parseLong(asset, 10), amount.toString()]	)
    }
...
//  TODO not supported
    ...
    public sendDividend(String sourceAddress, String dividend_asset, BigDecimal total_quantity) {
		return sendRPCMessage('sendtoowners_MP',[sourceAddress, Long.parseLong(dividend_asset, 10), total_quantity.toString()])
	}
